...
class TestGetNDCG(unittest.TestCase):
    def test_get_ndcg(self):
        ...
        expected_ndcg = {
            'q1': compute_expected_ndcg([1, 0, 1, 0], cutoff_ranks[0]),
            'q2': compute_expected_ndcg([0, 1, 0], cutoff_ranks[0]),
            'q3': compute_expected_ndcg([0, 1, 1], cutoff_ranks[0]),
        }

        expected_mean_ndcg = sum(expected_ndcg.values()) / len(expected_ndcg)

        # Call the function
        result = get_ndcg(qid2positive, qid2ranking, cutoff_ranks)

        # Check if the result is correct
        self.assertAlmostEqual(result[f'ndcg@{cutoff_ranks[0]}'], expected_mean_ndcg, places=4)
