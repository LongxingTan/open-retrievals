...
class RerankerInput(BaseModel):
    ...
    instruction: Optional[str] = Field(
        default="Given the user query, retrieval the relevant passages",
        description="Instruction for reranking"
    )
    ...
    async def reranker(self, request: RerankerInput) -> RerankerResponse:
        ...
        try:
            ...
            if len(request.questions) != len(request.texts):
                raise HTTPException(
                    status_code=400,
                    detail="Number of questions must match number of texts"
                )
            ...
            if len(request.questions) > 100:
                raise HTTPException(
                    status_code=400,
                    detail="Too many pairs. Maximum 100 question-text pairs per request."
                )
